...
#%%describing
mtcarsDF.shape #no: of rows and columns
mtcarsDF.head(3) #by specifying the number=3 in brackets, first three rows
mtcarsDF.tail(4) #last 4 data sets
mtcarsDF.describe()
mtcarsDF.columns
mtcarsDF.dtypes

mtcarsDF.index  #here index by rownames name of each row
type(mtcarsDF)

mtcarsDF.select_dtypes(include=['int64'])
mtcarsDF.select_dtypes(exclude=['int64'])
mtcarsDF.isna() #missing value
mtcarsDF.notna() #no missing value
id(mtcarsDF) #data frame identity
mtcars.empty  #if anything empty
mtcars.size
mtcars.ndim
mtcars.axes
mtcars.values
...
#%%% access DF
mtcarsDF[0:5]
#mtcarsDF[0:5:1,0:3:1]  #some error in the line
...
mtcarsDF.iloc[1:10:2, 1:5:2] #last digit is for alternate
mtcarsDF.iloc[1::2, 1::2] #skips alternate row.
